...
export async function PATCH(
  ...
  try {
    ...
    if (targetId === user.id) return err("Cannot deactivate yourself");
    ...
    await prisma.user.update({ where: { id: targetId }, data: { is_active: false } });
    return ok(null, "User deactivated");
  } catch (e) {
    console.error(e);
