...
export async function POST(request: NextRequest, { params }: { params: Promise<{ projectId: string; siId: string }> }) {
  ...
  await prisma.siteInspection.update({
    where: { id: parseInt(siId) },
    data: { status: "submitted" },
  });
  return ok(null, "Site inspection submitted");
}
